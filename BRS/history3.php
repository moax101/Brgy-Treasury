            ...
            <div class="col-8"> 
                    ...
                    $conn = mysqli_connect("localhost", "root", "", "store");
                        if($conn ->connect_error){
                            ...
                            $sql = " SELECT * from history3 ";
                            $sth = $conn->query($sql);
                            while ($row = mysqli_fetch_array($sth)) {   
                                echo "<tr><td>". $row["surname"]. "</td><td>". $row["status"]. "</td><td>". $row["brgy"]. "</td><td>" . $row["transaction3"]."</td><td>". $row["payment3"]."</td><td>". $row["exec_time"]. "</td><td>". $row["pending3"]. "</td><td> <form method='POST' action='#'>
                        ...
                        ?>
                            <?php
                                    if(isset($_POST["update"])){
                                        ...
                                        $sth = $db->query($sql);
                                        while ($row = mysqli_fetch_array($sth)) { 
                                        if($row['pending3']=="Pending") {                                          
                                            $insert = $db->query("UPDATE payment3 SET pending3='Already Taken' ");
                                            if($insert){
                                        ...
                                        {                                          
                                            $insert = $db->query("UPDATE payment3 SET pending3='Pending' ");
                                            if($insert){
                                                header("location:history3.php");
